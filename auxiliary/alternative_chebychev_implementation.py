...
class CCMethod:
    ...
    def __init__(self, a, b, n, func):
        ...
        cheb_nodes = [np.cos(np.pi * (k + 0.5) / n ) * bma + bpa for k in range(n)]      # Formula of nodes
        
        f = list(map(func, cheb_nodes))                                                 # y values of nodes                                              
        self.c = [(2.0 / n ) * sum([f[k] * np.cos(np.pi * j * (k + 0.5) / n )
                  for k in range(n)]) 
                  for j in range(n)]
    ...
    def eval(self, x):
        ...
        assert(self.a <= x <= self.b), 'x is not in defined interval, choose x between a and b'   # x picked from interval
        
        z = 2.0 * (x - self.a) / (self.b - self.a) - 1      # define z to normalize domain to [-1, 1]
        z2 = 2.0 * z
...
def approximation_error(func, approx, figure, degree):
    ...
    plt.ylabel("Error")
    plt.xlabel("$x$ - values")
    plt.title(f' Figure {figure}: Error Term of Chebyshev Approximation for {degree} Degrees')
    ...
    print(f"The maximal error of the Chebyshev interplation equals: {max(error_term):.5f}")
...
def data_frame_rslt(chep_approx, x, function):
    ...
    result = pd.DataFrame([approx, real_values, error], 
             index=['Approximation', 'Real Value', 'Absolut Error Term'],    
             columns=list(x))
    return result.rename_axis('$Y$-Values / $X$-Values').style.set_caption('Table 6: Absolut Error of Interpolation for different values')   # Style DataFrame
