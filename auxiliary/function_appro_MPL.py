...
def plot_train_test(x_train, x_test, y_train, y_test, prediction, xlabel, ylabel, num1, num2, method):
    ...
    train_marker = mlines.Line2D([], [], color='orange', marker='x', linestyle='None',
                          markersize=4, label='Test Data')
    test_marker = mlines.Line2D([], [], color='blue', marker='o', linestyle='None',
    ...
    if isinstance(prediction, np.ndarray) == False:
        ...
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
        plt.legend(handles=[train_marker, test_marker], 
                   title='Data',
...
class MLP3D:
    ...
    def __init__(self, func, x = [], xy = [], zvals = []):
        ...
        for i in range(len(self.x)):
            self.xy.append([(j,k) for j in self.x[i]            
                                  for k in self.x[i]])
    ...
    def train_test_method(self, ts):
        ...
        self.x_train0, self.x_test0, self.y_train0, self.y_test0 = train_test_split(self.xy[0], 
                                                                                    self.zvals[0], 
                                                                                    test_size = ts)
        self.x_train1, self.x_test1, self.y_train1, self.y_test1 = train_test_split(self.xy[1], 
                                                                                    self.zvals[1], 
                                                                                    test_size = ts)
        self.x_train2, self.x_test2, self.y_train2, self.y_test2 = train_test_split(self.xy[2], 
                                                                                    self.zvals[2], 
                                                                                    test_size = ts)
        self.x_train3, self.x_test3, self.y_train3, self.y_test3 = train_test_split(self.xy[4], 
                                                                                    self.zvals[4], 
                                                                                    test_size = ts)
        self.x_train, self.x_test, self.y_train, self.y_test = train_test_split(self.xy[3], 
                                                                                self.zvals[3], 
                                                                                test_size = ts)
    ...
    def error(self, caption):
        ...
        columns = ['Mean Absolute Error','Mean Squared Error', 'Explained Variance Score', '$R^2$ Score']
        ind = [str(len(self.xy[0])), str(len(self.xy[1])), str(len(self.xy[2])), str(len(self.xy[3])), str(len(self.xy[4]))]
        Y = [self.y_test0, self.y_test1, self.y_test2, self.y_test3, self.y_test]
        P = [self.predictions0, self.predictions1, self.predictions2, self.predictions3, self.predictions]
        ...
        rslt = pd.DataFrame([mae[i:i + 4] for i in range(0, len(mae), 4)], 
                            columns = columns,
    ...
    def pltres(self, fs, num):
        ...
        # plot approximation
        ax.scatter(x1_vals, x2_vals, self.predictions, c='red', marker='x', label='Approximation')
