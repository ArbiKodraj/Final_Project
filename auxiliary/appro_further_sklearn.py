...
class Regressor:
    ...
    def __init__(self, a, b, nodes, ts):
        ...
        y = np.piecewise(x, [x < 0,  (x >= 0.2) & (x < 0.5), (x >= 0.5) & (x < 0.8), x >= 0.8], 
                            [lambda x: -x + 5, 0, lambda x: x - 5, lambda x: np.exp(x)]).reshape(-1,1)
    ...
    def plot_pred(self, num):
        ...
        plt.plot(self.X_test, self.pred2, '*', ms = 5, color='c', label='Prediction KNN Regressor')
        plt.plot(self.X_test, self.pred1, 'x', ms = 4, color='r', label='Prediction Decision Tree Regressor', alpha = 0.7)
        plt.legend(bbox_to_anchor=(1.05, 1), 
                        ...
                        borderaxespad=0,
                        title_fontsize=12)
        plt.title(f'Figure {num}: Approximation of discontinuous functions via KNN and DT Regressor')
        plt.grid()
        plt.show()
    ...
    def create_error_df(self, num):
        ...
        for p in [self.pred1, self.pred2, self.pred3]:
            ...
            df = pd.DataFrame([l[:4], l[4:8], l[8:12]], 
        ...
        return df.style.set_caption(f'Table {num}: Approximation accuracy for different Methods')