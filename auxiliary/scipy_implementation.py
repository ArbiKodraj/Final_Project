...
g = lambda x: np.sin(.5 * x) - 2 * np.cos(2 * x)
...
class FCMethod:
    ...
    def plot_naive_interp(self, N, fs, number_1, number_2):
        ...
        ax0.plot(self.x, self.approx(self.x, *self.popta), label = str(self.n) + ' Nodes Approximation')
        ax0.plot(self.x, self.approx(self.x, *self.poptb), label = str(3*self.n) + ' Nodes Approximation')
        ax0.plot(self.x, self.approx(self.x, *self.poptc), label = str(9*self.n) + ' Nodes Approximation')
        ax0.set_title(f'Figure {number_1}: Naive Approximation Output ' + str(self.degree) + ' Degree')
        plt.grid()
        plt.legend(title='Naive Approximation for different Nodes',
        ...
        ax1.plot(self.x, self.approx(self.x, *self.popta)-self.func(self.x), label = str(self.n) + ' Nodes Error') 
        ax1.plot(self.x, self.approx(self.x, *self.poptb)-self.func(self.x), label = str(3*self.n) + ' Nodes Error') 
        ax1.plot(self.x, self.approx(self.x, *self.poptc)-self.func(self.x), label = str(9*self.n) + ' Nodes Error') 
        ax1.set_title(f'Figure {number_2}: Naive Approximation Error ' + str(self.degree) + ' Degree')
    ...
    def table_error(self, number):
        ...
        ev = explained_variance_score(self.approx(self.x, *self.popta), self.func(self.x))
        eva = explained_variance_score(self.approx(self.x, *self.poptb), self.func(self.x))
        evb = explained_variance_score(self.approx(self.x, *self.poptc), self.func(self.x))
        ...
        rslt = pd.concat([df, dfa, dfb], axis = 1).style.set_caption(f'Table {number}: Accuracy of Naive Approximation for ' + str(self.degree) + ' Degrees')
        return rslt
