...
def poly(a, b, n, x):
    ...
    z = 2 * (x - a) / (b - a) - 1
    T = 2 * z * np.cos(np.arccos(z) * (np.arange(0, n) - 1)) - np.cos(np.arccos(z) * (np.arange(0, n) - 2))
    return T
...
def show_cheb_polyn(a, b, n):
    ...
    [plt.plot(x, np.cos(np.arccos(x) * j), label = f'Polynomial for {j} degree') for j in range(1, 7)]
...
class ChebyNodeInterpolation:
    ...
    def plot_cn_interp(self, N, fs, number_1, number_2):
        ...
        ax0.plot(self.x, self.approx(self.x, *self.poptb), label = str(10 + self.n) + ' Nodes Approximation')
        ...
        ax1.plot(self.x, self.approx(self.x, *self.poptb)-self.func(self.x), label = str(10 + self.n) + ' Nodes Error') 
    ...
    def table_error(self, number):
        ...
        eva = explained_variance_score(self.approx(self.x, *self.popta), self.func(self.x))
        evb = explained_variance_score(self.approx(self.x, *self.poptb), self.func(self.x))
        ...
        rslt = pd.concat([dfa, dfb], axis = 1).style.set_caption(f'Table {number}: Accuracy of Monomial Interpolation with Chebychev-nodes for ' + str(self.degree) + ' Degrees')
        return rslt
