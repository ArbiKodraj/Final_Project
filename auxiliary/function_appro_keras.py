...
class SeqMethod:
    ...
    def seq_init(self, activator_in, activator_out):
        ...
        self.model.add(Dense(6, input_dim=1, activation=activator_in, kernel_initializer='he_uniform')) #50
        self.model.add(Dense(4, activation=activator_in, kernel_initializer="he_uniform")) #40
        self.model.add(Dense(4, activation=activator_in, kernel_initializer="he_uniform")) #30
    ...
    def predict_test(self, a, b, ts):
        ...
        y = self.func(X)
        self.x_train, self.x_test, self.y_train, self.y_test  = train_test_split(X, y, test_size = ts)
        callback = EarlyStopping(monitor="val_loss", patience=100, restore_best_weights=True)
        self.model.fit(self.x_train, self.y_train, 
                        validation_data=(self.x_test, self.y_test), 
                        epochs=self.epochs, callbacks=[callback], verbose=False)
...
class _Outcome(SeqMethod): 
    ...
    def plt_approximation(self, num):
        train_marker = Line2D([], [], color='orange', marker="o", linestyle='None',
                          markersize=4, label='Test Data')
        test_marker = Line2D([], [], color='blue', marker='v', linestyle='None',
                        markersize=4, label='Train Data')
        approx_marker = Line2D([], [], color='red', marker='*', linestyle='None',
                        markersize=4, label='Keras Output')
...
def plt_side_by_side(num1, num2, num3, 
    ...
    *arg1, *arg2, arg3*  : data of first, second and third function
    ...
    ax3.grid()
    ax3.legend(handles=[train_marker, test_marker, approx_marker], 
...
def gen_frame(true_list, pred_list, num):
        ...
        rslt = pd.DataFrame([accuracy[i:i + 4] for i in range(0, len(accuracy), 4)],
                            ...
                            index = ['$e^{-x}$',
                                     '$|x|$',
                                     '$x sin(\pi x)$'])
        return rslt.style.set_caption(f'Table {num}: Approximation-Accuracy of Keras Method for different functions').apply(highlight_min, subset=['Mean Absolute Error', 'Mean Squared Error'])
...
class MultidimApprox:
    ...
    def estimator(self, epochs, bs, vs):
        estimator = KerasRegressor(build_fn=baseline_model, batch_size=bs, verbose=False)
        self.hist = estimator.fit(self.X_train, self.y_train, epochs=epochs, validation_split=vs)
    ...
    def plt_first_rslt(self, num):
        ...
        ax1.legend(title = 'Loss', loc = 'upper center', 
                   bbox_to_anchor=(0.5, 1.1),
                  ncol=3, title_fontsize=12,
        ...
        ax2.legend(title='Data', loc='upper center', 
                   bbox_to_anchor=(0.5, 1.1),
    ...
    def plt_second_rslt(self, num1, num2):
        ...
        ax1.hlines(abs(error).mean(), 0, 52, ls='-.', lw=2, color = 'c', label='Mean Absolute Error')
        ax1.hlines((error ** 2).mean(), 0, 52, ls='dotted', lw=2, color = 'r', label='Mean Squared Error')
        ax1.set_xlim([0,50])
        ...
        mpl_table = ax2.table(cellText = error_df.values.round(5), 
