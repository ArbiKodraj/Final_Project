...
class StockPricePredictor(object):
    ...
    def _data_prep(self, variable='Close', **kwargs):
        ...
        self.X_train, self.X_test, self.y_train, self.y_test = train_test_split(time, price, **kwargs)
    ...
    def plot_data(self, neigh=2, fs=(15,7), variable='Close', num=18.1, **kwargs):
        ...
        assert (X_train.shape == y_train.shape) and (X_test.shape == y_test.shape), 'Shapes are not correct!'
        self.mod = knn(n_neighbors=neigh, 
                  weights='uniform', 
                  algorithm='auto', 
                  leaf_size=30, 
                  p=2, 
        ...
        for i, (train, test) in enumerate(zip([self.X_train, self.X_test, self.X_test], [self.y_train, self.y_test, y_pred])):
    ...
    def _predict_price_knn(self, weight='distance', neigh=2, days=3, periods=30):
        ...
        predict_model = knn(n_neighbors=neigh, 
                            weights=weight, 
                            algorithm='auto', 
                            leaf_size=30, 
                            p=2, 
    ...
    def nn_accuracy(self, **kwargs):
        ...
        X_train = np.array(pd.to_datetime(pd.Series(self.X_train.ravel())).map(datetime.toordinal))
        X_test = np.array(pd.to_datetime(pd.Series(self.X_test.ravel())).map(datetime.toordinal))
        mlp.fit(X_train.reshape(-1, 1), self.y_train.reshape(-1, 1).ravel())
        pred = mlp.predict(X_test.reshape(-1, 1))
        return print(f'Accuracy of MLP Regressor in terms of RSME: {mean_squared_error(self.y_test, pred, squared=False)}')
...
class NetworkPricePredictor(StockPricePredictor):
    ...
    def assign_data(self, plot=True, num=18.3):
        ...
        if plot == True:
            ...
            plt.legend()
            plt.grid(True)
            plt.title(f'Figure {num}: Visualization of Train and Test Data for MLP Regression')
            plt.show()
        elif plot == False:
    ...
    def mlp_prediction(self, plot=True, num=18.4, **kwargs): 
        ...
        plt.show()
        return print(f'RMSE of prediction equals: {np.mean(np.abs(prediction - y_test))}')
...
def get_actual_price_stock(link):
    ...
    price = float(hist.find_all("div", {"class" : "D(ib) Va(m) Maw(65%) Ov(h)"})[0].find("span").text)
